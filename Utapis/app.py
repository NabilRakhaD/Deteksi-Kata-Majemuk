...
def index():
    ...
    if request.method == 'POST':
        ...
        if len(df_1kata)>0:
            ...
            #df2_1kata = df2_1kata[~df2_1kata['Majemuk terdeteksi'].isin(df0_1kata['Majemuk terdeteksi'])]
            #df2_1kata = df2_1kata[~df2_1kata['Majemuk terdeteksi'].isin(df1_1kata['Majemuk terdeteksi'])]
            #df2_1kata = df2_1kata.groupby(['Majemuk terdeteksi', 'Value']).agg({"Majemuk Koreksi": ", ".join,}).reset_index()
            #df2_1kata = df2_1kata.sort_values(by=['Value'], ascending=False)
            #df2_1kata = df2_1kata.drop_duplicates(subset=['Majemuk terdeteksi'])

            #df3_1kata = df3_1kata[~df3_1kata['Majemuk terdeteksi'].isin(df0_1kata['Majemuk terdeteksi'])]
            #df3_1kata = df3_1kata[~df3_1kata['Majemuk terdeteksi'].isin(df1_1kata['Majemuk terdeteksi'])]
            #df3_1kata = df3_1kata[~df3_1kata['Majemuk terdeteksi'].isin(df2_1kata['Majemuk terdeteksi'])]
            #df3_1kata = df3_1kata.groupby(['Majemuk terdeteksi', 'Value']).agg({"Majemuk Koreksi": ", ".join,}).reset_index()
            #df3_1kata = df3_1kata.sort_values(by=['Value'])
            #df3_1kata = df3_1kata.drop_duplicates(subset=['Majemuk terdeteksi'])

            #kata_majemuk_benar_1kata = df0_1kata['Majemuk terdeteksi'].values.tolist()
            kata_majemuk_salah_1kata = df1_1kata['Majemuk terdeteksi'].values.tolist()
            #kata_majemuk_salah_1kata = kata_majemuk_salah_1kata_temp['Majemuk terdeteksi'].values.tolist()
        ...
        #df2 = df2[df2['Value'].apply(lambda x: x >= 0.83)]
        #df2 = df2.sort_values(by=['Value'])
        #df2 = df2.drop_duplicates(subset=['Majemuk terdeteksi'])

        #df3 = df3[df3['Value'].apply(lambda x: x >= 0.79)]
        ...
        return render_template('result.html',  kalimat=kalimat, result=result)
